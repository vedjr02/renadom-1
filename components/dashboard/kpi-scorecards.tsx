  ...
  return (
    <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
      {cards.map((card) => (
        <Card
          key={card.label}
          className="glass-panel border-white/10 bg-transparent p-0 ring-0"
        >
          <Text className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
            {card.label}
          </Text>
          <Metric className="mt-3 text-foreground">{card.value}</Metric>
          <Text className="mt-1 text-xs text-muted-foreground">{card.delta}</Text>
